...
//estimate
allele_stat estimate (Locus &site, models &model, std::vector<float_t> &gofs, const args &arg){
	...
	mle.ll=model.loglikelihood(site, mle, arg.minimum_coverage);		//Sets the site.ll to the log likelihood of the best fit (ll). 
	...
	temp.freq=1.;
	temp.f=0.;
	mle.monoll=model.loglikelihood(site, temp, arg.minimum_coverage);			//Sets the site.ll to the log likelihood of the best fit (ll). 
	...
	mle.hwell=model.loglikelihood(site, temp, arg.minimum_coverage);		//?
	return mle;
}
...
int estimateInd(int argc, char *argv[])
{
	...
	env.optional_arg('m',"minerror", &arg.minimum_error, 	&arg_setfloat_t, "please provide a float.", "prior estimate of the error rate (defualt 0.001).");
	env.optional_arg('M',"mincoverage", &arg.minimum_coverage, 	&arg_setint, 	"please provide an int.", "minimum coverage of sites to be estimated (defualt 4).");
	env.optional_arg('a',"alpha", 	&arg.alpha,		&arg_setfloat_t, "please provide a float.", "cut-off value for printing polymorphic sites (default 0.0).");
	...
	env.flag(	'V', "verbose", &arg.verbose,	&flag_set, 	"an error occured while enabeling verbose excecution.", "prints more information while the command is running.");
	env.flag(	'q', "quite", 	&arg.quite,	&flag_set, 	"an error occured while enabeling quite execution.", "prints less information while the command is running.");
		...
		#pragma omp parallel private(c, estimate_me) 
		{
			...
			for (uint32_t x=0; x<BUFFER_SIZE; ++x){
				...
				if(estimate_me) {
					...
					buffer_mle[c]=estimate (buffer_site[c], model, gofs, arg);
					#pragma omp critical
