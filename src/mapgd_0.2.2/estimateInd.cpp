...
//estimate
allele_stat estimate (site_t &site, models &model, count_t MIN, count_t EMLMIN, count_t MAXGOF, count_t MAXPITCH){
	...
	maximizegrid(site, mle, model, MIN, MAXGOF, MAXPITCH+texc);	//trim bad clones and re-fit the model.
...
#pragma omp parallel private(c, model)
{
#pragma omp for
...
#pragma omp critical
{
			...
			if(pro.read(buffer_site[c])!=EOF) readed++;	//reads the next line of the pro file. pro.read() retuerns 0
}
			buffer_mle[c]=estimate (buffer_site[c], model, MIN, EMLMIN, MAXGOF, MAXPITCH);
		}
}
