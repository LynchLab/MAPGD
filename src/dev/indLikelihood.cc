...
float_t models::loglikelihood(site_t const &site, allele_stat const &p, count_t const &MIN){
...
/*@Breif, a function that calculats the log likelihood of a set of observations. */
float_t models::genotypelikelihood(quartet_t const &quartet, allele_stat const &population, count_t const &MIN){
...
float_t models::lnP(count_t const *base, allele_stat const &p){
...
void EVofP (count_t N_, allele_stat const &a, models &model, float_t &E, float_t &V){
	count_t l[4];
	float_t tP;
	E=0; V=0;
...
float_t gof (site_t &site, allele_stat const &a, models &model, count_t const &MIN, float_t const &site_maxgof){
...
count_t maximizegrid (site_t &site, allele_stat &a, models &model, count_t const &MIN, float_t const &maxgof, count_t const &maxpitch){
...
//		std::cout << maxll_ << ", "<< PtQ << ", " << PtH << "( " << P_ << " : " << H_ << " : " << Q_ << ")" << std::endl;
	...
	a.gof=gof(site, a, model, MIN, maxgof);
...
//	std::cout << a.error << "-" << N_ << "::" << a.freq << std::endl;
	...
	if ( abs(a.gof)>maxgof) {
		...
		return maximizegrid(site, a, model, MIN, maxgof, maxpitch);
	};
