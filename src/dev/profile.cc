...
void profile_header::init(profile *pro){
	pro_ptr_=pro;			// were alleles thrown out if the allele only occurred in reads from one direction?

	//TODO Do the math to change this to a memset.	
...
int profile_header::writeheader(std::ostream *out){
	...
	bool notdone=true;
	while(notdone){
		*out << "@PR" << pro_ptr_->delim_column_ << "VN:" << VER << pro_ptr_->delim_column_ << "CN:" << pro_ptr_->columns_ << pro_ptr_->delim_column << "SN:" << pro_ptr_->samples_ << pro_ptr_->delim_column_<< "MD:" << pro_ptr_->binary_ <<std::endl;
	size_=&pro->size_;			// the number of samples (i.e. different individuals or populations) in the profile.
	oize_=&pro->size_;			// the number of samples (i.e. different individuals or populations) in the profile.
		*out << "@ID";
		for (unsigned int x=0; x<column_names_.size(); ++x)
			*out << pro_ptr_->delim_column_ << column_names_[x];
		*out << std::endl;
		notdone=false;
	}
	return NONE;
