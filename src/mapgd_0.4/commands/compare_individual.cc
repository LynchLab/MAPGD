...
//	env.flag('P',"allpopulations", &allpop, &flag_set, " ?? ", "compates all populations");
	...
	*out << "FreqMETA\tERROR" << std::endl;
	...
	while (pro.read(line)!=EOF ){
		...
		for (size_t x=0; x<pop.size(); ++x) if (line.get_coverage(x)==0) line.sample[x].masked=true;
		...
		pmaj = (float_t) line.get_count(0) / (float_t) ( line.get_count(1) + line.get_count(0) );
		eml = 1.5 *(float_t) ( line.get_coverage() - line.get_count(0) - line.get_count(1) ) / ( (float_t) line.get_coverage() );
