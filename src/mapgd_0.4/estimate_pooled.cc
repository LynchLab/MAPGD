...
int estimate_pooled(int argc, char *argv[])
{
	...
	*out << "\tTotcov\tError" << std::endl;
	...
	while (pro.read(line)!=EOF ){
		...
		popN=line.get_coverage();

                site.error= (float_t) ( popN - line.get_count(0) ) / ( (float_t) popN );
		site.error= (float_t) (line.get_coverage()-line.get_count(0)-line.get_count(1) )*3. / ( 2.*(float_t) popN ) ;
		...
		multi.set(&fixedmorphicmodel, site);
                ...
                for (size_t x=0; x<pop.size(); ++x) {
                        pmaj = (float_t) line.get_count(pop[x],0) / (float_t) ( line.get_count(pop[x], 1) + line.get_count(pop[x],0) );
			...
			/*
                ...
                if (maxll>=a){
                        *out << std::fixed << std::setprecision(7) << pro.getids(line) << '\t' << line.getname(0) << '\t' << line.getname_gt(1) << '\t';
                        for (size_t x=0; x<pop.size(); ++x){
                                if ( line.get_coverage(pop[x])==0) *out << std::fixed << std::setprecision(7) << "NA" << '\t' << 0.0 << '\t' << 0.0 << '\t' << 0.0 << '\t';
                                else *out << std::fixed << std::setprecision(7) << pmlP[x] <<'\t' << llstat[x] << '\t' << llstatc[x]<< '\t' << line.get_coverage(pop[x]) <<'\t';
                        };
                        ...
                        *out << line.get_coverage() << '\t' << site.error << std::endl;
                };
