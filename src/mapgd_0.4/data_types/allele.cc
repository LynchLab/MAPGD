...
const std::string
Allele::sql_get_constructor(void) const {
	return "PRAGMA table_info('GENOME')";
}
...
Allele::sql_read(std::istream& in){
	...
	int junk;
	line_stream >> junk; ref=junk;
	line_stream >> junk; major=junk;
	line_stream >> junk; minor=junk;
	line_stream >> coverage;
	...
	hwell=ll-hwell/2.;
	freq=MM+Mm/2;
	f=(2*freq*(1-freq)-Mm)/(2*freq*(1-freq) );
	error=0;
	null_error=0;
	null_error2=0;
}
...
const std::string Allele::sql_header(void) const {
	...
	if (!print_bias)
		...
		return "(ABS_POS int, REF int, MAJOR int, MINOR int, COVERAG int, HOM_FREQ REAL, HET_FREQ REAL, POLY_LR REAL, HWE_LR REAL, GOF REAL, EF_CHRM REAL, IND_INC int, IND_CUT int, MJ_BIAS REAL, P_MJ_BS REAL, BEST_LL REAL, PRIMARY KEY (ABS_POS) )";
}
...
const std::string Allele::sql_column_names(void) const {
	if (!print_bias)
		...
		return "(ABS_POS, REF, MAJOR, MINOR, COVERAG, HOM_FREQ, HET_FREQ, POLY_LR, HWE_LR, GOF, EF_CHRM, IND_INC, IND_CUT, MJ_BIAS, P_MJ_BS, BEST_LL)";
}
...
const std::string Allele::sql_values(void) const {
	...
	snprintf(return_buffer, SQL_LINE_SIZE, "(%d, %d, %d, %d, %d, %Lf, %Lf, %Lf, %Lf, %Lf, %Lf, %d, %d, %Lf, %Lf, %Lf)",
	abs_pos_,
	ref,
	major,
	minor,
	...
	snprintf(return_buffer, SQL_LINE_SIZE, "(%d, %d, %d, %d, %d, %f, %f, %f, %f, %f, %f, %d, %d, %f, %f, %f)",
	abs_pos_,
	ref,
	major,
	minor,
