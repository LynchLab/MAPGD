...
const bool File_index::print_once=true;
...
int
File_index::add_id(const std::string & id_str, const id1_t &length)
{
	size_.push_back(length);
	cumulative_size_.push_back(cumulative_size_.back()+length);
	last_id0_str_=id_str;
	last_id0_=id0_.size();
	id0_str_[last_id0_str_]=last_id0_;
	id0_.push_back(last_id0_str_);
	return 0;
...
size_t
File_index::get_names_size (void) const
{
	return id0_.size();				
}	
