...
for i in range (0, 3):
	for j in range (0, 3):
		...
		for y in range(0, NUMBER_REP):
			os.system("python simulate_random_IBD.py -c "+str(COV[i])+" -s 192.00 -N "+str(NUMBER_LOCI)+" -M "+str(CM[j])+" > sim/temp.txt")
		...
		N=0
		for y in range(0, NUMBER_REP):
			os.system("python simulate_random_IBD.py -c "+str(COV[i])+" -s 188.00 -I True -N "+str(NUMBER_LOCI)+" -M "+str(CM[j])+" > sim/temp.txt")
			File=open("sim/temp.txt")
			E=map(float, File.readline().split()[0:11])
			E2=map(float, File.readline().split()[0:11])
			File.close()
...
print "\\begin{table}"
print "\\caption{. \\textbf{Bias ($\\times 10^3$) and MSE ($\\times 10^4$) of mapgd with and without linkage} in estimating coancestry ($\\Theta$), cofraternity ($\\Delta_{\\ddot X \\ddot Y}$), and inbreeding ($f_X$) for outbred and inbred siblings. The expected value of all 7 IBD coefficients are listed in table \\ref{tab:ibdexamples}. Observed values of coefficients are calculated based off of actual genome sharing. Results are from "+str(NUMBER_REP)+" simulations of "+str(NUMBER_LOCI)+" SNPs distributed evenly across "+str(14)+" chromosomes of varying genetic size.}"
print "\\label{tab:linkage_sim}"
print "\\begin{tabular}{l*{13}{c}r}"
print "\\hline"
print "\\textbf{Coancestry}  &  & \\multicolumn{5}{c}{Expected Value} & & \\multicolumn{5}{c}{Sample Value} \\\\" 
print "\\cline {3-7}"
print "\\cline {9-13}"
print  " &  & \\multicolumn{2}{c}{outbred ($\\Theta=\\frac {1}{4}$)} & & \\multicolumn{2}{c}{inbred ($\\Theta=\\frac {1}{2}$)}&  & \\multicolumn{2}{c}{outbred ($\\Theta=\\frac {1}{4}$)} & & \\multicolumn{2}{c}{inbred ($\\Theta=\\frac {1}{2}$)}\\\\"
print  "\cline {3-4}"
print "\\cline {6-7}"
print "\\cline {9-10}"
print "\\cline {12-13}"
...
for i in range (0, 3):
	for j in range (0, 3):
		print CM[j], "&", COV[i], "$\\times$ &", round( (A[i][j][3])*10**3,2), "&" , round( (B[i][j][3])*10**4, 2), "&&", round( (AI[i][j][3])*10**3,2), "&" , round( (BI[i][j][3])*10**4, 2), "&&", round( (A2[i][j][3])*10**3,2), "&" , round( (B2[i][j][3])*10**4, 2), "&&", round( (AI2[i][j][3])*10**3,2), "&" , round( (BI2[i][j][3])*10**4, 2), "\\\\"
	print "\\hline"

print "\\hline"
print  "\\textbf{Cofraternity} &  & \\multicolumn{2}{c}{outbred ($\\Delta=\\frac {1}{4}$)} & & \\multicolumn{2}{c}{inbred ($\\Delta=\\frac {3}{8}$)}&  & \\multicolumn{2}{c}{outbred ($\\Delta=\\frac {1}{4}$)} & & \\multicolumn{2}{c}{inbred ($\\Delta=\\frac {3}{8}$)}\\\\"
print  "\cline {3-4}"
print "\\cline {6-7}"
print "\\cline {9-10}"
print "\\cline {12-13}"
...
for i in range (0, 3):
	for j in range (0, 3):
		print CM[j], "&", COV[i], "$\\times$ &", round( (A[i][j][7])*10**3,2), "&" , round( (B[i][j][7])*10**4, 2), "&&", round( (AI[i][j][7])*10**3,2), "&" , round( (BI[i][j][7])*10**4, 2), "&&", round( (A2[i][j][7])*10**3,2), "&" , round( (B2[i][j][7])*10**4, 2), "&&", round( (AI2[i][j][7])*10**3,2), "&" , round( (BI2[i][j][7])*10**4, 2), "\\\\"
	print "\\hline"
...
for i in range (0, 3):
	for j in range (0, 3):
		print CM[j], "&", COV[i], "$\\times$ &", round( (A[i][j][1]+A[i][j][2])*10**3/2.,2), "&" , round( (B[i][j][1]+B[i][j][2])/2.*10**4, 2), "&&", round( (AI[i][j][1]+A2[i][j][2])/2.*10**3,2), "&" , round( (BI[i][j][1]+B2[i][j][2])/2.*10**4, 2), "&&", round( (A2[i][j][1]+A2[i][j][2])*10**3/2.,2), "&" , round( (B2[i][j][1]+B2[i][j][2])/2.*10**4, 2), "&&", round( (AI2[i][j][1]+AI2[i][j][2])/2.*10**3,2), "&" , round( (BI2[i][j][1]+BI2[i][j][2])/2.*10**4, 2), "\\\\"
	print "\\hline"
print "\\end{tabular}%"
print "\\end{table}"
